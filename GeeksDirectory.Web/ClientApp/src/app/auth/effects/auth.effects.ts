...
export class AuthEffects {
  ...
  restore$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(() => {
        if (
          this.storageService.existsAuthToken() &&
          this.storageService.existsAuthUser()
        ) {
  ...
  openDialog$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(action =>
        this.authDialog.signIn({ ...action["credentials"] })
      ),
      map(result => {
      ...
      mergeMap(({ credentials }) =>
        this.authService
          ...
          .pipe(
            ...
            catchError(() =>
              of(SignInDialogActions.openDialog({ credentials }))
            )
          )
      )
    )
  ...
  signInSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.authService
          .getMyProfile()
          .pipe(map(profile => AuthApiActions.personalizeSuccess({ profile })))
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ profile }) => {
        ...
        return this.authService
          ...
          .pipe(
            map(() => AuthApiActions.registerProfileSuccess({ credentials }))
          );
      })
    )
  );
