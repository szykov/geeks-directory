    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.requestService
          ...
          .pipe(
            map(result =>
              ProfilesApiActions.loadProfilesSuccess({ collection: result })
            )
          )
      )
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ profileId }) =>
        this.requestService
          ...
          .pipe(
            map(result =>
              ProfilesApiActions.loadProfileDetailsSuccess({ selected: result })
            )
          )
      )
    )
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ profileId, model }) =>
        this.requestService.updateProfile(profileId, model).pipe(
          ...
          map(result =>
            ProfilesApiActions.updateProfileSuccess({ selected: result })
          )
        )
      )
    )
