...
export const reducer = createReducer(
  ...
  on(
    SkillsApiActions.evaluateSkillSuccess,
    (state, { skillName, averageScore }) => {
      let selected = { ...state.selected };

      let targetIndex = selected.skills.findIndex(
      ...
      selected.skills = [
        ...selected.skills.slice(0, targetIndex),
        updatedSkill,
        ...selected.skills.slice(targetIndex + 1)
      ];
